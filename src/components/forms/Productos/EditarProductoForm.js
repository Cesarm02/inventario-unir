...
export default function EditarProductoForm({ errors, onSubmitCallback, id }) {
    ...
    axios
      ...
      .then((response) => {
        ...
        if(response.data.codigo!=null){
            ...
            setCodigoProducto("A123")
        }
        console.log(response.data)
        //Imprimir el producto
        const aux = {
          value: response.data.categoria.id,
          label: response.data.categoria.nombre,
        };
        setSelectCat(aux);
  ...
  const changeCantidad = (cantidad) => {
    ...
    const changeCodigo = (codigo) => {
    let codigoProducto = codigo.target.value;
    ...
    <Form onSubmit={submitForm}>
      ...
      <Form.Group>
        <Form.Group control="categoria">
          ...
          <Select options={listOptionCate} onChange={categoriaChanges}></Select>
        </Form.Group>
      </Form.Group>
      <Row>
        <Col md="6" xs="12">
        ...
        <Col md="6" xs="12">
          <Form.Group control="estado">
            ...
            <Select
              options={listOption}
              defaultValue={selectEst}
              onChange={estadoChanges}
            ></Select>
          </Form.Group>
        </Col>
      </Row>
      <Row>
        <Col md="6" xs="12">
          <Form.Group control="unidad">
            ...
            <Select options={listUnidad} onChange={unidadChange} />
          </Form.Group>
        </Col>
        <Col md="6" xs="12">
